...
import { pb, pbAuth } from "@/lib/serverHelpers";
...
export const GET: RequestHandler = async (event) => {
  ...
  await pbAuth()
  const results = await pb.collection("legacy_keys").getFullList({ filter: (pb.filter("key = {:givenKey}", { givenKey })) })
  if (results.length == 0) {
    return sendJson({ "error": "No such key", valid: false }, 404)
  }
